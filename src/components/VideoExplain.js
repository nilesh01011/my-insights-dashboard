...
function VideoExplain() {
    ...
    return (
        <>
            <div onClick={() => setIsOpenVideo(true)} className='flex items-center justify-end w-full'>
            ...
            {/* video contents */}
            <div className={`w-full h-full z-[999999] py-[65px] px-[25px] max-w-[1260px] mx-auto fixed top-0 left-0 right-0 bottom-0 ${isOpenVideo === true ? 'block' : 'hidden'} transition-all duration-300`}>
                {/* title and cancel icons */}
                <div className='w-full flex items-center justify-between'>
                    {/* title */}
                    <h1 className='text-[20px] font-bold text-white z-[999999]'>How to add insight</h1>
                    {/* icons */}
                    <button onClick={() => setIsOpenVideo(false)} aria-label="sidebar button" type='button' className='sidebar-btn w-[28px] z-[999999] flex group items-center flex-col gap-[6px] cursor-pointer'>
                ...
                {/* video images */}
                <div id='animationShow' className={`w-full h-full flex items-center justify-center ${isOpenVideo === true ? 'animatedShow' : ''}`}>
                    {/* thumbnail */}
                    <div className='w-max h-max relative cursor-pointer'>
                        {/* thumbnail images */}
