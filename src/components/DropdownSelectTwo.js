...
function DropdownSelectTwo({ items, placeholder, mandatory, disable }) {
            ...
            <div ref={domNode} className={`w-full h-[48px] rounded-[5px] text-[#635D5D] flex flex-col items-center ${disable === true ? 'bg-[#F4F4F4]' : 'bg-white'} border-[1px] border-[#DEDEDE]`}>
                ...
                {/* all lists items */}
                {
                    disable === false && isOpen && (
                        <ul className={`z-20 divide-y-[1px] border-[1px] bg-[#F4F4F4] border-[#DEDEDE] divide-[#DEDEDE] ${isOpen ? 'opacity-1 duration-[1500ms]' : 'opacity-0 duration-[1500ms]'} relative w-full left-0 right-0 md:-top-[330%] -top-[300%] rounded-[5px] transition-all`}>
                            {items?.map((ele, id) => {
...
export default DropdownSelectTwo