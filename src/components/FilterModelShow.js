...
function FilterModelShow({ filterModelShow, setFilterModelShow, }) {
    ...
    return (
        <>
            ...
            {/* {
                ...
                <div onClick={() => filterModelShow(false)} className='w-full h-screen fixed top-[64px] left-0 right-0 bottom-0 bg-black/80 z-[9999] cursor-pointer'>
                </div>
            } */}

            {/* sidebar contents */}
            <div ref={domNode} id='animateScroll' className={`bg-white ${filterModelShow === true && 'animateScroll'} z-[9999] p-[12px_20px] shadow-2xl`} style={{ transition: 'all 0.4s cubic-bezier(0.18, 0.89, 0.43, 1.19)' }}>
                {/* head */}
                <div className='w-full flex items-center justify-between pb-[20px]'>
                    {/* title */}
                    <h1 className='text-[20px] font-bold'>Filter</h1>
                    {/* icons */}
                    <button onClick={() => setFilterModelShow(false)} type='button'>
                ...
                <div className='fixed bottom-0 left-0 right-0 w-full h-max flex items-center justify-between p-[16px] bg-white' style={{ boxShadow: '-5px 0px 50px 0px rgba(0, 0, 0, 0.2)' }}>
                    {/* left button */}
                    <button onClick={() => setFilterModelShow(false)} type='button' className='p-[6px_16px] w-max h-max text-[#FF3E5B] font-[500] text-[14px] hover:bg-[#FF3E5B] hover:text-white rounded-[4px] border-[1px] border-[#FF3E5B]'>Cancel</button>
                    {/* right side button */}
                    <div className='w-max flex items-center gap-[8px]'>
                        ...
                        <button onClick={() => setFilterModelShow(false)} type='button' className='p-[6px_16px] w-max h-max text-white bg-[#FF3E5B] font-[500] text-[14px] rounded-[4px] border-[1px] border-[#FF3E5B]'>Apply</button>
                    </div>
                </div>
            </div>
        </>
...
export default FilterModelShow